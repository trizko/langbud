...
def docker_pg():
    ...
    container = client.containers.run(
        'postgres',
        name='test-postgres',
        environment={'POSTGRES_PASSWORD': 'password', 'POSTGRES_USER': 'user', 'POSTGRES_DB': 'testdb'},
        ports={'5432/tcp': 5432},
        detach=True
    )
...
async def db_pool(docker_pg):
    ...
    os.system('dbmt up --db-url postgresql://user:password@localhost:5432/testdb --migrations-dir migrations')
